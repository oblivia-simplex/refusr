...
Population(
    ...
    options::Options,
    nfeatures::Int,
) where {T<:Real} = Population(
    [
        PopMember(dataset, baseline, genRandomTree(nlength, options, nfeatures), options) for i = 1:npop
    ],
    npop,
)
"""
...
function finalizeScores(
    ...
    if need_recalculate
        @inbounds @simd for member = 1:pop.n
            pop.members[member].score =
                scoreFunc(dataset, baseline, pop.members[member].tree, options)
        end
    end
    return pop
end
...
function bestSubPopParetoDominating(
    pop::Population{T};
    topn::Int = 10,
)::Population where {T<:Real}
