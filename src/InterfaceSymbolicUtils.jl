...
const SYMBOLIC_UTILS_TYPES =
    Union{<:Number,SymbolicUtils.Sym{<:Number},SymbolicUtils.Term{<:Number}}
...
function node_to_symbolic(
    ...
    elseif tree.degree == 1
        ...
        if (op in (cos, sin, exp, cot, tan, csc, sec)) || evaluate_functions
            ...
            if index_functions
                dummy_op = SymbolicUtils.Sym{(SymbolicUtils.FnType){Tuple{Number},Real}}(
                    Symbol("_unaop$(tree.op)"),
                )
            else
                dummy_op = SymbolicUtils.Sym{(SymbolicUtils.FnType){Tuple{Number},Real}}(
                    Symbol(op),
        ...
        if (op in (+, -, *, /)) || evaluate_functions
            ...
            if index_functions
                dummy_op =
                    SymbolicUtils.Sym{(SymbolicUtils.FnType){Tuple{Number,Number},Real}}(
                        Symbol("_binop$(tree.op)"),
                    )
            else
                dummy_op =
                    SymbolicUtils.Sym{(SymbolicUtils.FnType){Tuple{Number,Number},Real}}(
...
function symbolic_to_node(
    ...
    options::Options;
    varMap::Union{Array{String,1},Nothing} = nothing,
)::Node where {T<:SymbolicUtils.Sym{<:Number}}
    return Node(varMap_to_index(eqn.name, varMap))
end

function _multiarg_split(
    op_idx::Int,
    eqn::Array{Any,1},
    options::Options,
