...
function multiply_powers(
    eqn::T,
)::SYMBOLIC_UTILS_TYPES where {T<:Union{<:Number,SymbolicUtils.Sym{<:Number}}}
    return eqn
end
...
function multiply_powers(
    eqn::T,
    op::F,
)::SYMBOLIC_UTILS_TYPES where {F,T<:SymbolicUtils.Term{<:Number}}
...
function multiply_powers(
    eqn::T,
)::SYMBOLIC_UTILS_TYPES where {T<:SymbolicUtils.Term{<:Number}}
...
function get_simplifier(binops::A, unaops::B) where {A,B}
    ...
    function number_simplifier()
        rule_tree =
            [
                If(istree, Chain(ASSORTED_RULES)),
                If(is_operation(+), Chain(PLUS_RULES)),
                If(is_operation(*), Chain(TIMES_RULES)),
    ...
    function default_simplifier(; kw...)
        IfElse(
            has_trig,
            Postwalk(Chain((number_simplifier(), trig_simplifier())), ; kw...),
    ...
    serial_polynormal_simplifier =
        If(istree, Fixpoint(Chain((polynormalize, Fixpoint(default_simplifier())))))
    return serial_polynormal_simplifier
end

function custom_simplify(
    init_eqn::T,
    options::Options,
)::SYMBOLIC_UTILS_TYPES where {T<:SYMBOLIC_UTILS_TYPES}
